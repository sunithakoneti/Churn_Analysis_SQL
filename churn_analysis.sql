...
use churn;
create table codebytes(id int, subscription_start date, subscription_end date,segment int);
...
select min(month(subscription_start)) from codebytes;
select max(month(subscription_start)) from codebytes;
select  distinct month(subscription_start) as range_of_months from codebytes where month(subscription_start) between 1 and 12 order by range_of_months asc;
...
('2017-03-01', '2017-03-31');
 ...
 select c.*,m.first_day,m.last_day from
 codebytes as c cross join months as m);
...
select
  ...
  and subscription_start < first_day then 1 else 0 end as is_active_87,
  case when segment = 30
  and subscription_start < first_day then 1 else 0 end as is_active_30
from
  cross_join;
